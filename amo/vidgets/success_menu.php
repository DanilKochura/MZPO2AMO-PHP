...
if(file_exists(__DIR__.'/success'.$date.$date1.'.txt') and !$_GET['refresh'])
{
    $data = json_decode(file_get_contents(__DIR__.'/success'.$date.$date1.'.txt'), true);
        ...
        foreach ($leadsClosed as $lead)
        {
            ...
            if($lead->created_at >= strtotime($date1) and $lead->created_at <= strtotime($date))
            {
                ...
                try {
                    ...
                    die("Возникла ошибка. ПОвторите позже.");
                }
                $next = false;
                foreach($events as $e)
                {
                    foreach($e->valueAfter as $statuse)
                    {
                        if($statuse['lead_status']['id'] == 32533204 or $statuse['lead_status']['id'] == 32533201)
                        {
//                            dd([$e, date("Y-m-d", $e->created_at)]);
                            if($e->created_at <= strtotime($date1) or $e->created_at >= strtotime($date))
                            {
                                $coco++;
                                $next = true;
                                break;
...
//endregion
    ...
    file_put_contents(__DIR__.'/success'.$date.$date1.'.txt', json_encode([$labels, $prices, $amount]), FILE_APPEND);
