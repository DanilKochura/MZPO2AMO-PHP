...
//dd($data);
if (file_exists('active_leads_korp_'.$date1.'-'.$date.'.json')) {
...
//		file_put_contents(__DIR__.'/avg_ret.json', json_encode($avg));
	...
	{
		$avg = json_decode(file_get_contents(__DIR__.'/active_leads_korp_'.$date1.'-'.$date.'.json'));
	}
} else
{

	$arr = [];
	foreach ($managers as $m => $n)
	{
		$cf = new \AmoCRM\Filters\LeadsFilter();
		$cf->setResponsibleUserId($m)->setLimit(200)->setUpdatedAt((new BaseRangeFilter())
			->setFrom(strtotime($date1))
			->setTo(strtotime($date)))->setUpdatedBy($n);
		$ll = 0;
		$lc = new LeadsCollection();
		$co = 0;
		$amo = true;
		$i = 0;
		$o = 0;
	...
	file_put_contents(__DIR__.'/active_leads_korp_'.$date1.'-'.$date.'.json', json_encode($avg));
