...
//region Измененные за период сделки на этапах оплаты (без предыдущих)
$filterUpdatedPaid = new LeadsFilter();
$leadNotesService = $apiClient->notes(EntityTypesInterface::LEADS);
$filterUpdatedPaid->setStatuses([
    [
        'status_id' => 32533204,
        'pipeline_id' => \MzpoAmo\Pipelines::RETAIL
    ],
...
])->setUpdatedAt((new BaseRangeFilter())
    ->setFrom(strtotime($date1))
    ->setTo(strtotime($date)))->setLimit(200);
$leadsUpdatedPaid = $apiClient->leads()->get($filterUpdatedPaid);
foreach ($leadsUpdatedPaid as $l)
{
    if($l->created_at >= strtotime($date1) and $l->created_at <= strtotime($date))
    {
       continue;
    } else
    {
        ...
        $notesCollection = $leadNotesService->getByParentId($l->getId());
            ...
            if($n->getText() == 'Изменён этап сделки')
            {
                if($n->created_at >= strtotime($date1) and $n->created_at <= strtotime($date) )
                {
                    ...
                    $releads[] =
                        [
                            'id' => $l->getId(),
                            'created' => $l->getCreatedAt()
                        ];

                    if(isset($month[(string)date("Y-m", $l->created_at)]))
                    {
                        $month[date("Y-m", $l->created_at)]['amount']++;
                        $month[date("Y-m", $l->created_at)]['sum']+=$l->getPrice();
                    } else
                    {
                        $month[date("Y-m", $l->created_at)]['amount'] = 1;
                        $month[date("Y-m", $l->created_at)]['sum'] = $l->getPrice();
                    }
                    break;
                }
