...
$contact = $apiClient->contacts()->getOne($id, [\AmoCRM\Models\ContactModel::LEADS]);

$response = [];


$pipelines = [\MzpoAmo\Pipelines::RETAIL, \MzpoAmo\Pipelines::POSTSAIL, \MzpoAmo\Pipelines::FARMERS];


foreach ($contact->getLeads() as $lead)
{
    $sync = new Leads([], MzpoAmo::SUBDOMAIN, $lead->getId());
    if(!in_array($sync->getLead()->getPipelineId(), $pipelines)) continue;
    $response[] = [
        'id' => $sync->getId(),
        'name' => $sync->getLead()->getName(),
        'course' => $sync->getCFValue(CustomFields::COURSE[0]),
        'price' => $sync->getPrice(),
        'status' => $sync->getStatus(),
        'sname' => $apiClient->statuses($sync->getLead()->getPipelineId())->getOne($sync->getStatus())->getName(),
        'scolor' => $apiClient->statuses($sync->getLead()->getPipelineId())->getOne($sync->getStatus())->getColor(),
        'pipe' => $apiClient->pipelines()->getOne($sync->getLead()->getPipelineId())->getName(),
        'resp' => $apiClient->users()->getOne($sync->getResponsible())->getName(),
        'resp_id' => $sync->getResponsible()
    ];
}
